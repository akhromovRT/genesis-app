...
### 2026-05-31 — Пересборка коммерческого слоя: модель «полный продукт + воронка» + 225 SNP

- **Источник:** материалы Галины из чата GenesisBio (27–29.05) — 5 DOCX с финальными rs-составами блоков, голосовое 8:41 с механикой воронки, текстовые правки. Заархивированы в `agent_docs/genetics-blocks/source-galina-2026-05/`. Спека: `docs/superpowers/specs/2026-05-31-vitrina-funnel-snp-design.md`; план: `docs/superpowers/plans/2026-05-31-vitrina-funnel-snp.md`.
- **Ветка:** `feat/vitrina-funnel-snp` (8 тасков, TDD, субагент-драйв с двухстадийным ревью).
- **Что сделано:**
  - **SNP источник истины** — `src/lib/genetics/blocks-snp.ts` + инвариант-тест (`blocks-snp.test.ts`). Все 5 блоков транскрибированы из DOCX.
  - **Миграция `0006`** — markersCount блоков 1–4 (51/28/27/12) и Трио (88); Блок 5 `consultation_hours=0` (консультация только в полном пакете) + копирайт без фикс. числа; пакет «Пошаговая сборка 75 000» деактивирован (`is_active=false`).
  - **Корзина** — чистая логика `src/lib/cart/rules.ts` (гейтинг воронки + подарок Блок 4) + подключение к `cart-store.ts`; жёсткий гейтинг (Блок 2/3/4 нельзя без Блока 1; Блок 5 и пакеты — исключение), авто-подарок Блок 4 при ≥2 блоках, тост-отказ.
  - **Витрина `/products`** — герой = полный продукт; секция блоков как воронка; **порядок блоков по коду** (BLOCK_1 первым — устранён баг «база за 14 900», возникавший из-за сортировки по цене); удалён блок «Почему Genesis, а не другая лаборатория» (Genesis — не лаборатория); терминология «ген-точки (SNP)» вместо «генов» / «поштучно».
  - **`/early-access`** — полный пакет 65 000 (было 55 900); «Шаг за шагом к персональному протоколу» (убрано «что со мной не так»).
  - **Страницы блоков `/products/[slug]`** — перечень ген-точек (SNP) из источника истины; чистка формулировки «в подарок».
- **Число SNP — РАЗРЕШЕНО (перечитыванием первоисточника):** блоки 1–4 сходятся точно как уникальные (51/28/27/12). Расхождение только в блоке 5: его фармакогенетические колонки = 122 записи → 103 уникальных (один SNP читается ради нескольких лекарств и считается несколько раз) + 18 усилителей из блоков 1–4 → её 146; декомпозиция 51+22+15+9+128 = 225. **Owner decision (2026-05-31): витрина и БД показывают честные уникальные числа (всего 184, блок 5 = 103); 225/146 Галины — задокументированы как авторский счёт «по контекстам», но не отображаются** (миграция `0007_honest_snp_counts.sql`, правки копирайта `/products`, `/products/full`, `/early-access`). Модуль = 184 (без изменений); тесты — 83/83 зелёные. См. ADR-003.
