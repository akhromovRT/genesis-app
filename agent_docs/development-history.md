...
- **⚠️ Открытое расхождение по числу SNP:** при честном подсчёте уникальных пар gene:rs панель = **184** (Блок 5 = 103), тогда как Галина заявляет **225** (Блок 5 = 146). Причина — Галина считает «по контекстам анализа» (один SNP в нескольких лекарственных колонках аптечки + пересечения блоков), а не уникальные SNP. **Решение владельца:** модуль хранит честные 184; маркетинговое «225» в существующем копирайте и `markersCount` Блока 5/полного пакета **не трогаем до уточнения у Галины** (методология vs докомплект SNP). См. ADR-003.
